...
resdir='test_rzwave_1step_results/'

loadct,39
set_device,resdir+'biermann_wave.eps',/eps
filename=resdir+'*/GM/z*.outs'
func='bz {bz}-0.05*!pi^2/10000*sin(!pi/10*x)*sin(!pi/10*y)/(1+0.1*cos(!pi/10*y))^2
...
.r getpict
wref=w
wref(*,*,6)=0.05*!pi^2/10000*sin(!pi/10*x(*,*,0))*sin(!pi/10*x(*,*,1))/(1+0.1*cos(!pi/10*x(*,*,1)))^2
...
.r getlog
plot_oo,1/wlog(*,0),wlog(*,1),xrange=[1e-2,1e-1],yrange=[5e-4,1e-1],$
        psym=-2,thick=4, $
        xtitle='Grid resolution', $
        ytitle='relative error', $
        title="Biermann wave-test"
oplot,1/wlog(*,0),wlog(*,2),psym=-4,thick=4
oplot,[1e-2,1e-1],[6e-3,6e-2],linestyle=2,thick=3
xyouts,0.03,0.05,'1st order slope',charsize=2,charthick=1
