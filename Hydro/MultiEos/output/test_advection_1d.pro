...
set_device,resdir+'godunov_limit_volume_lookup.eps',/epe, /land
.r animate
close_device,/pdf
...
printf,99, $
       'n godunov g_lim g_noncons g_lim_mix g_lim_mix_look g_lim_vol_look ' + $
       ' linde l_lim l_noncons l_lim_mix l_lim_mix_look l_lim_vol_look'
...
.r getpict
errors(3)=rel_error(w0,wref,[0,1,4,5])
errors(4)=rel_error(w1,wref,[0,1,4,5])
errors(5)=rel_error(w2,wref,[0,1,4,5])

filename=resdir+'Linde_40_/GM/c*.outs ' $
  +      resdir+'Linde_40_LIMIT/GM/c*.outs ' $
  +      resdir+'Linde_40_NONCONS/GM/c*.outs'
.r getpict
errors(6)=rel_error(w0,wref,[0,1,4,5])
errors(7)=rel_error(w1,wref,[0,1,4,5])
errors(8)=rel_error(w2,wref,[0,1,4,5])

filename=resdir+'Linde_40_LIMIT_*/GM/c*.outs'
.r getpict
errors(9)=rel_error(w0,wref,[0,1,4,5])
errors(10)=rel_error(w1,wref,[0,1,4,5])
errors(11)=rel_error(w2,wref,[0,1,4,5])
...
oplot,4/wlog(*,0),wlog(*,5),psym=-6,thick=3
oplot,4/wlog(*,0),wlog(*,6),psym=-7,thick=3
...
oplot,[0.0015],[0.005],psym=7 & xyouts,0.002,0.005,'+ #VOLUME + #LOOKUP'
